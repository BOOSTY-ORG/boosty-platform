...
const ExportModal = ({ isOpen, onClose, investorIds = [], currentFilters = {} }) => {
  ...
  const [exportScope, setExportScope] = useState(investorIds.length > 0 ? 'selected' : 'filtered');
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setSelectedColumns([
        'investorId', 'firstName', 'lastName', 'email', 'phone', 
        'status', 'kycStatus', 'totalInvestment', 'joinedDate'
      ]);
    }
  }, [isOpen]);
  ...
  const handleExport = async () => {
    ...
    try {
      const exportOptions = {
        format: exportFormat,
        scope: exportScope,
        investorIds: exportScope === 'selected' ? investorIds : undefined,
      ...
      if (saveAsTemplate && templateName) {
        await investorsAPI.createExportTemplate({
          name: templateName,
          format: exportFormat,
          columns: selectedColumns,
          includeRelatedData,
      ...
      link.href = url;
      link.setAttribute('download', `investors_export_${new Date().toISOString().split('T')[0]}.${exportFormat}`);
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            <label className="flex items-center">
              ...
              />
              <span>Selected Investors ({investorIds.length})</span>
            </label>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            <label className="flex items-center">
              <input
                type="checkbox"
                checked={includeRelatedData.investments}
            ...
            <label className="flex items-center">
              <input
                type="checkbox"
                checked={includeRelatedData.transactions}
            ...
            <label className="flex items-center">
              <input
                type="checkbox"
                checked={includeRelatedData.kycDocuments}
