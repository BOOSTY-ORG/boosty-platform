...
const ExportTemplate = ({ isOpen, onClose, onTemplateSelect }) => {
  ...
  const handleCreateTemplate = async () => {
    ...
    try {
      await usersAPI.createExportTemplate(formData);
      showNotification({
        type: 'success',
  ...
  const handleUpdateTemplate = async () => {
    ...
    try {
      await usersAPI.updateExportTemplate(editingTemplate._id, formData);
      showNotification({
        type: 'success',
  ...
  const handleBulkDelete = async () => {
    try {
      await Promise.all(selectedTemplates.map(id => usersAPI.deleteExportTemplate(id)));
      showNotification({
        type: 'success',
      ...
      <div>
        ...
        <div className="space-y-2">
          <label className="flex items-center">
            <input
              type="checkbox"
              checked={formData.includeRelatedData.applications}
          ...
          <label className="flex items-center">
            <input
              type="checkbox"
              checked={formData.includeRelatedData.installations}
          ...
          <label className="flex items-center">
            <input
              type="checkbox"
              checked={formData.includeRelatedData.communications}
          ...
          <label className="flex items-center">
            <input
              type="checkbox"
              checked={formData.includeRelatedData.documents}
