/**
 * Transaction Analytics Load Test Scenario
 *
 * This file contains specific load testing scenarios for the transaction analytics endpoint
...
class TransactionAnalyticsTest {
  constructor() {
    ...
    this.endpoint = '/api/metrics/transaction/analytics';
    this.authToken = null;
  }
  ...
  async testTransactionAnalytics(options = {}) {
    const {
      concurrentUsers = 10,
      totalRequests = 100,
      duration = 30,
    ...
    console.log(`\n🚀 Testing Transaction Analytics Endpoint`);
    console.log(
    ...
    try {
      ...
      console.log(`\n✅ Transaction Analytics test completed`);
      console.log(
      ...
      console.error(`❌ Transaction Analytics test failed:`, error.message);
  ...
  async runComparisonTests() {
    console.log('\n🔄 Running comparison tests for Transaction Analytics...');
    ...
    const testVariations = [
      ...
      { period: '7d', groupBy: 'day', includeProjections: 'true' },
      { period: '30d', groupBy: 'week', includeProjections: 'true' },
      { period: '7d', groupBy: 'day', transactionType: 'investment' },
      { period: '30d', groupBy: 'week', transactionType: 'payout' },
      { period: '7d', groupBy: 'day', transactionType: 'withdrawal' },
      { period: '30d', groupBy: 'week', refreshCache: 'true' },
    ];

    // Test with caching
    console.log('\n💾 Testing with caching enabled...');
    const withCachingResults = await this.testTransactionAnalytics({
      concurrentUsers: 50,
      totalRequests: 500,
      duration: 30,
      withCaching: true,
    ...
    const withoutCachingResults = await this.testTransactionAnalytics({
      concurrentUsers: 50,
      totalRequests: 500,
      duration: 30,
      withCaching: false,
...
export default TransactionAnalyticsTest;

// Run if this file is executed directly
async function main() {
  const test = new TransactionAnalyticsTest();

  try {
    // Run comparison tests
    const results = await test.runComparisonTests();

    console.log('\n🎉 Transaction Analytics load testing completed!');
    console.log('📄 Results available in the returned object');
  } catch (error) {
    console.error('💥 Transaction Analytics load testing failed:', error);
    process.exit(1);
  }
}
