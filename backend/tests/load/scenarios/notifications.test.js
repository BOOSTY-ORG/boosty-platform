/**
 * Notifications Load Test Scenario
 *
 * This file contains specific load testing scenarios for the notifications endpoint
...
class NotificationsTest {
  ...
  async runComparisonTests() {
    console.log('\n🔄 Running comparison tests for Notifications...');
    ...
    const testVariations = [
      ...
      { page: 1, limit: 20 },
      { page: 2, limit: 10 },
      { page: 1, limit: 10, type: 'system' },
      { page: 1, limit: 10, type: 'transaction' },
      { page: 1, limit: 10, type: 'kyc' },
      { page: 1, limit: 10, read: 'false' },
      { page: 1, limit: 10, priority: 'high' },
      { page: 1, limit: 10, refreshCache: 'true' },
    ];

    // Test with caching
    console.log('\n💾 Testing with caching enabled...');
    const withCachingResults = await this.testNotifications({
      concurrentUsers: 50,
      totalRequests: 500,
      duration: 30,
      withCaching: true,
    ...
    const withoutCachingResults = await this.testNotifications({
      concurrentUsers: 50,
      totalRequests: 500,
      duration: 30,
      withCaching: false,
...
async function main() {
  ...
  try {
    ...
    console.log('\n🎉 Notifications load testing completed!');
    console.log('📄 Results available in the returned object');
  } catch (error) {
    console.error('💥 Notifications load testing failed:', error);
    process.exit(1);
  }
}
