/**
 * Dashboard Metrics Load Test Scenario
 *
 * This file contains specific load testing scenarios for the dashboard metrics endpoint
...
class DashboardMetricsTest {
  constructor() {
    ...
    this.endpoint = '/api/metrics/dashboard';
    this.authToken = null;
  }
  ...
  async testDashboardMetrics(options = {}) {
    const {
      concurrentUsers = 10,
      totalRequests = 100,
      duration = 30,
    ...
    try {
      ...
      console.error(`❌ Dashboard Metrics test failed:`, error.message);
  ...
  async runComparisonTests() {
    console.log('\n🔄 Running comparison tests for Dashboard Metrics...');
    ...
    const testVariations = [
      ...
      { period: '30d', includeProjections: 'true' },
      { period: '7d', refreshCache: 'true' },
      { period: '30d', refreshCache: 'true' },
    ];

    // Test with caching
    console.log('\n💾 Testing with caching enabled...');
    const withCachingResults = await this.testDashboardMetrics({
      concurrentUsers: 50,
      totalRequests: 500,
      duration: 30,
      withCaching: true,
    ...
    const withoutCachingResults = await this.testDashboardMetrics({
      concurrentUsers: 50,
      totalRequests: 500,
      duration: 30,
      withCaching: false,
...
export default DashboardMetricsTest;
...
async function main() {
  ...
  try {
    ...
    console.log('\n🎉 Dashboard Metrics load testing completed!');
    console.log('📄 Results available in the returned object');
  } catch (error) {
    console.error('💥 Dashboard Metrics load testing failed:', error);
    process.exit(1);
  }
}
