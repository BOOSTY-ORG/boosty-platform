/**
 * Investor Metrics Load Test Scenario
 *
 * This file contains specific load testing scenarios for the investor metrics endpoint
...
class InvestorMetricsTest {
  constructor() {
    this.baseURL = 'http://localhost:7000';
    this.endpoint = '/api/metrics/investor';
    this.authToken = null;
  }
  ...
  async testInvestorMetrics(options = {}) {
    const {
      concurrentUsers = 10,
      totalRequests = 100,
      duration = 30,
    ...
    console.log(`\n🚀 Testing Investor Metrics Endpoint`);
    console.log(
    ...
    try {
      ...
      console.log(`\n✅ Investor Metrics test completed`);
      console.log(
      ...
      console.error(`❌ Investor Metrics test failed:`, error.message);
  ...
  async runComparisonTests() {
    console.log('\n🔄 Running comparison tests for Investor Metrics...');
    ...
    const testVariations = [
      ...
      { period: '7d', groupBy: 'day', includeRoi: 'true' },
      { period: '30d', groupBy: 'week', includeRoi: 'true' },
      { period: '7d', groupBy: 'day', investorType: 'individual' },
      { period: '30d', groupBy: 'week', investorType: 'institutional' },
      { period: '7d', groupBy: 'day', minInvestment: '10000' },
      { period: '30d', groupBy: 'week', refreshCache: 'true' },
    ];

    // Test with caching
    console.log('\n💾 Testing with caching enabled...');
    const withCachingResults = await this.testInvestorMetrics({
      concurrentUsers: 50,
      totalRequests: 500,
      duration: 30,
      withCaching: true,
    ...
    const withoutCachingResults = await this.testInvestorMetrics({
      concurrentUsers: 50,
      totalRequests: 500,
      duration: 30,
      withCaching: false,
...
export default InvestorMetricsTest;

// Run if this file is executed directly
async function main() {
  const test = new InvestorMetricsTest();

  try {
    // Run comparison tests
    const results = await test.runComparisonTests();

    console.log('\n🎉 Investor Metrics load testing completed!');
    console.log('📄 Results available in the returned object');
  } catch (error) {
    console.error('💥 Investor Metrics load testing failed:', error);
    process.exit(1);
  }
}
