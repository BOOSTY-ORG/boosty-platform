...
export const getUserMetrics = async (req, res) => {
  try {
    ...
    // Use cache-aside pattern for user metrics
        ...
        const response = {
          summary: {
            ...
            averageApplicationsPerUser:
              userApplications.totalApplications / totalUsers || 0,
...
export const getUserDetails = async (req, res) => {
  try {
    ...
    // Use cache-aside pattern for user details
    return await cacheUserData(
      `details:${userId}`,
      async () => {
        ...
        const transactions = await Transaction.find({
          $or: [{ fromEntityId: user._id }, { toEntityId: user._id }],
          createdAt: { $gte: startDate, $lte: endDate },
        }).sort({ createdAt: -1 });

        // Calculate detailed metrics
        const detailedMetrics = await calculateDetailedUserMetrics(
          user,
          applications,
          kycDocuments,
          transactions
...
export const getUserList = async (req, res) => {
  try {
    const { startDate, endDate } = parseDateRange(req.query);
    const cacheKey = `list:${startDate.toISOString()}:${endDate.toISOString()}:${JSON.stringify(req.query)}`;
      ...
      async () => {
        ...
        const usersWithMetrics = await Promise.all(
          users.map(async (user) => {
            const [applicationCount, kycCount, transactionCount] =
              await Promise.all([
...
export const getUserActivityMetrics = async (req, res) => {
  try {
    const { startDate, endDate } = parseDateRange(req.query);
    const cacheKey = `activity:${startDate.toISOString()}:${endDate.toISOString()}`;

    // Use cache-aside pattern for user activity metrics
    return await cacheUserData(
      'activity',
      async () => {
...
const getUserActivityStats = async () => {
  ...
  return await cacheQueryResult(
    ...
    {
      ttl: 5 * 60, // 5 minutes cache for activity stats
...
const getUserApplicationStats = async () => {
  ...
  return await cacheQueryResult(
    ...
    {
      ttl: 10 * 60, // 10 minutes cache for application stats
      tags: ['user', 'application', 'stats'],
    }
  );
};

const getUserKYCStats = async () => {
  const cacheKey = 'kyc:stats';
  ...
  return await cacheQueryResult(
    ...
    {
      ttl: 15 * 60, // 15 minutes cache for KYC stats
      tags: ['user', 'kyc', 'stats'],
    }
  );
};
...
const getUserTransactionStats = async () => {
  const cacheKey = 'transaction:stats';

  return await cacheQueryResult(
    cacheKey,
    async () => {
      const [transactionCount, totalVolume, completedTransactions] =
        await Promise.all([
          Transaction.countDocuments({
            createdAt: {
              $gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
      ...
      const averageValue =
        transactionCount > 0
          ? (totalVolume[0]?.total || 0) / transactionCount
          : 0;
      const completionRate =
        transactionCount > 0
          ? (completedTransactions / transactionCount) * 100
          : 0;
    ...
    {
      ttl: 10 * 60, // 10 minutes cache for transaction stats
      tags: ['user', 'transaction', 'stats'],
    }
  );
};
...
const getUserTrends = async () => {
  ...
  return await cacheQueryResult(
    ...
    },
    {
      ...
      tags: ['user', 'trends', 'analytics'],
    }
  );
};

const getUserEngagementMetrics = async () => {
  const [usersWithApplications, usersWithKYC, usersWithTransactions] =
    await Promise.all([
      SolarApplication.distinct('userId', {
        createdAt: { $gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) },
      }),
      KYCDocument.distinct('userId', {
        uploadedAt: { $gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) },
      }),
      Transaction.distinct('fromEntityId', {
        createdAt: { $gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) },
      }),
    ]);
  ...
  return {
    applicationEngagement:
      totalUsers > 0 ? (usersWithApplications.length / totalUsers) * 100 : 0,
    kycEngagement:
      totalUsers > 0 ? (usersWithKYC.length / totalUsers) * 100 : 0,
    transactionEngagement:
      totalUsers > 0 ? (usersWithTransactions.length / totalUsers) * 100 : 0,
  };
};

const calculateDetailedUserMetrics = async (
  user,
  applications,
  kycDocuments,
  transactions
  ...
  const installedApplications = applications.filter(
    (app) => app.applicationStatus === 'installed'
  ).length;

  const verifiedDocuments = kycDocuments.filter(
    (doc) => doc.verificationStatus === 'verified'
  ).length;
  const pendingDocuments = kycDocuments.filter(
  ...
  const kycCompletionRate =
    kycDocuments.length > 0
      ? (verifiedDocuments / kycDocuments.length) * 100
      : 0;
...
export const invalidateUserData = async (userId) => {
  try {
    if (userId) {
      await invalidateUserCache(userId);
    } else {
      // Invalidate all user cache if no specific ID
      await invalidateUserCache('all');
    }
  } catch (error) {
