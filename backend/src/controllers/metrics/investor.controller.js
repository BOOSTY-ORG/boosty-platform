...
export const getInvestorMetrics = async (req, res) => {
  try {
    ...
    // Use cache-aside pattern for investor metrics
    return await cacheInvestorData('metrics', async () => {
      ...
      const [
        ...
        Investor.aggregate([
          ...
          {
            $group: {
              _id: '$investmentPreferences.preferredRegions',
              count: { $sum: 1 },
            },
          },
        ]),
      ...
      const response = {
        summary: {
          ...
          averageInvestmentPerInvestor:
            investmentMetrics.totalInvestments / totalInvestors || 0,
        ...
        topInvestors: topInvestors.map((investor) => ({
          ...
          roi: investor.roi,
          activeInvestments: investor.performance?.activeInvestments || 0,
        })),
    ...
    console.error('[DEBUG] General error in getInvestorMetrics:', error);
...
export const getInvestorDetails = async (req, res) => {
  try {
    ...
    const transactions = await Transaction.find({
      $or: [{ fromEntityId: investor._id }, { toEntityId: investor._id }],
      createdAt: { $gte: startDate, $lte: endDate },
    }).sort({ createdAt: -1 });

    // Calculate detailed metrics
    const detailedMetrics = await calculateDetailedInvestorMetrics(
      investor,
      investments,
      transactions
    );
...
export const getInvestorList = async (req, res) => {
  try {
    ...
    // FIX: Correct operator precedence for pagination calculation
...
export const getInvestorPerformanceMetrics = async (req, res) => {
  try {
    ...
    const durationAnalysis = await Investment.aggregate([
      {
        $match: {
          investorId: { $in: await Investor.find(query).distinct('_id') },
        },
      },
      {
        $group: {
...
const getInvestmentMetrics = async (query) => {
  const cacheKey = `investment:${JSON.stringify(query)}`;
  ...
  return await cacheQueryResult(
    ...
    async () => {
      ...
      const [totalInvestments, totalReturns, activeInvestments] =
        await Promise.all([
          Investor.aggregate([
    ...
    {
      tags: ['investor', 'investment', 'metrics'],
    }
  );
};

const getPerformanceMetrics = async (query, startDate, endDate) => {
  const cacheKey = `performance:${JSON.stringify(query)}:${startDate.toISOString()}:${endDate.toISOString()}`;
  ...
  return await cacheQueryResult(
    ...
    async () => {
      ...
      const defaultRate =
        totalInvestments > 0
          ? (defaultedInvestments / totalInvestments) * 100
          : 0;
    ...
    {
      tags: ['investor', 'performance', 'metrics'],
    }
  );
};
...
const calculateDetailedInvestorMetrics = async (
  investor,
  investments,
  transactions
) => {
  ...
  const totalInvestmentAmount = investmentTransactions.reduce(
    (sum, t) => sum + t.amount,
    0
  );
  const totalRepaymentAmount = repaymentTransactions.reduce(
    (sum, t) => sum + t.amount,
    0
  );
  ...
  // FIX: Add null checks for performance properties
  const onTimePayments = investments.reduce((sum, inv) => {
    return sum + (inv.performance?.onTimePayments || 0);
  }, 0);

  const latePayments = investments.reduce((sum, inv) => {
    return sum + (inv.performance?.latePayments || 0);
  }, 0);
...
export const updateInvestor = async (req, res) => {
  try {
    ...
    const investor = await Investor.findByIdAndUpdate(investorId, updateData, {
      new: true,
      runValidators: true,
    });
...
export const deleteInvestor = async (req, res) => {
  try {
    ...
    return res.json(
      formatSuccessResponse({ message: 'Investor deleted successfully' }, req)
    );
  } catch (error) {
    return handleControllerError(error, req, res);
  }
...
export const uploadKYCDocument = async (req, res) => {
  try {
    ...
    const { default: KYCDocument } = await import(
      '../../models/metrics/kycDocument.model.js'
    );
...
export const getInvestorKYCDocuments = async (req, res) => {
  try {
    ...
    const documents = await KYCDocument.find({ userId: investorId }).sort({
      uploadedAt: -1,
    });
...
export const bulkExportInvestors = async (req, res) => {
  try {
    ...
    if (format === 'csv') {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="investors_export.csv"`
      );
      return res.send(csv);
    } else if (format === 'xlsx') {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="investors_export.json"`
      );
  ...
  try {
    ...
    const templates = [
      ...
      },
      {
        ...
        description:
          'Full investor data including investments and transactions',
        format: 'xlsx',
        columns: [
          'investorId',
          'firstName',
...
export const exportInvestorsAdvanced = async (req, res) => {
  try {
    ...
    if (format === 'csv') {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="investors_export_${new Date().toISOString().split('T')[0]}.csv"`
      );
      return res.send(csv);
    } else if (format === 'xlsx') {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="investors_export_${new Date().toISOString().split('T')[0]}.json"`
      );
  ...
  try {
    ...
    if (dateFrom) {
      filteredExports = filteredExports.filter(
        (exp) => exp.createdAt >= new Date(dateFrom)
      );
    }
    if (dateTo) {
      filteredExports = filteredExports.filter(
        (exp) => exp.createdAt <= new Date(dateTo)
      );
    }
...
export const getInvestorPerformanceAnalytics = async (req, res) => {
  try {
    ...
    console.log(
      `Getting performance analytics for investor: ${investorId}, period: ${period}`
    );
    const analyticsData = {
      period,
      performance: {
      ...
      recommendations: [
        {
          ...
          description:
            'Consider adding investments in different sectors to reduce concentration risk',
        ...
        },
        {
          ...
          description:
            'Current market conditions show increased volatility, consider defensive positions',
        ...
        },
        {
          ...
          description:
            'Regular portfolio rebalancing can help maintain optimal risk-return profile',
  ...
  try {
    ...
    const timelineData = {
      ...
      monthlyActivity: [
        ...
        { month: 'May', investments: 0, returns: 637.5, net: -637.5 },
        ...
        { month: 'Nov', investments: 0, returns: 637.5, net: -637.5 },
      ...
      cashFlow: [
        {
          ...
          type: 'deposit',
          amount: 15000,
          description: 'Initial investment in Solar Farm A',
        },
        {
          date: '2023-03-22',
          type: 'deposit',
          amount: 8000,
          description: 'Investment in Rooftop Solar C',
        },
        {
          date: '2023-05-10',
          type: 'withdrawal',
          amount: 637.5,
          description: 'Quarterly return from Solar Farm A',
        },
        {
          date: '2023-06-18',
          type: 'deposit',
          amount: 7000,
          description: 'Investment in Community Solar D',
        },
        {
          date: '2023-08-05',
          type: 'withdrawal',
          amount: 144,
          description: 'Quarterly return from Rooftop Solar C',
        },
        {
          date: '2023-10-28',
          type: 'deposit',
          amount: 5000,
          description: 'Investment in Commercial Solar E',
        },
        {
          date: '2023-11-15',
          type: 'withdrawal',
...
// Helper function to invalidate investor cache when data changes
export const invalidateInvestorData = async (investorId) => {
  try {
    if (investorId) {
      await invalidateInvestorCache(investorId);
    } else {
      // Invalidate all investor cache if no specific ID
      await invalidateInvestorCache('all');
    }
  } catch (error) {
    console.error('Error invalidating investor cache:', error);
  }
};
...
export default {
  ...
  getInvestorPerformanceAnalytics,
