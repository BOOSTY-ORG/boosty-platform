...
const TEST_CONFIG = {
  ...
  endpoints: {
    ...
    investors: [
      ...
      {
        path: '/metrics/investors/test-investor-id',
        method: 'GET',
        requiresAuth: true,
    ...
    kyc: [
      {
        path: '/metrics/investors/test-investor-id/kyc',
        method: 'POST',
        requiresAuth: true,
      ...
      },
      {
        ...
        method: 'GET',
        requiresAuth: true,
        description: 'Get investor KYC documents',
      },
      {
        path: '/metrics/investors/test-investor-id/kyc/test-document-id/verify',
        method: 'PUT',
        requiresAuth: true,
        description: 'Verify KYC document',
      },
      {
        path: '/metrics/investors/test-investor-id/kyc/test-document-id/reject',
        method: 'PUT',
        requiresAuth: true,
      ...
      },
      {
        ...
        method: 'GET',
        requiresAuth: true,
        description: 'Get KYC metrics',
      },
      {
        path: '/metrics/kyc/list',
        method: 'GET',
        requiresAuth: true,
        description: 'List KYC documents',
      },
    ],
  },
};
...
async function authenticateUser() {
  ...
  const loginResult = await makeRequest(TEST_CONFIG.endpoints.auth[1], {
...
async function testEndpoint(endpoint, category, token = null) {
  ...
  if (endpoint.method === 'POST' && endpoint.path === '/api/users') {
  ...
  } else if (endpoint.method === 'POST' && endpoint.path === '/auth/login') {
    testData = {
      email: 'test@example.com',
      password: 'password123',
    };
    ...
    endpoint.method === 'POST' &&
    endpoint.path === '/reset-password/test-token'
  ) {
    testData = {
    ...
    endpoint.method === 'POST' &&
    endpoint.path.includes('/metrics/investors')
  ) {
    testData = {
      firstName: 'Test',
      lastName: 'Investor',
  ...
  if (result.success) {
    ...
    console.log(
      `✅ PASSED - Status: ${result.status}, Time: ${result.responseTime}ms`
    );
    ...
    if (
      endpoint.path === '/metrics/investors' &&
      endpoint.method === 'POST' &&
      result.data.data
    ) {
    ...
    console.log(
      `⏭️  SKIPPED - Authentication required (Status: ${result.status})`
    );
    return testResult;
  } else {
    testResults.failed.push(testResult);
    console.log(
      `❌ FAILED - Status: ${result.status}, Error: ${result.error || 'Unknown error'}`
    );
    if (result.data) {
...
function generatePerformanceReport() {
  ...
  const slowEndpoints = testResults.performance.filter(
    (p) => p.responseTime > slowThreshold
  );
...
function generateSummary() {
  ...
  console.log(
    `Success Rate: ${((testResults.passed.length / total) * 100).toFixed(2)}%`
  );
  ...
  categories.forEach((category) => {
    const categoryPassed = testResults.passed.filter(
      (t) => t.category === category
    ).length;
    const categoryFailed = testResults.failed.filter(
      (t) => t.category === category
    ).length;
    const categorySkipped = testResults.skipped.filter(
      (t) => t.category === category
    ).length;
    ...
    console.log(
      `     Total: ${categoryTotal}, Passed: ${categoryPassed}, Failed: ${categoryFailed}, Skipped: ${categorySkipped}`
    );
  });
  ...
  if (testResults.failed.length > 0) {
    ...
    testResults.failed.forEach((test) => {
      console.log(
        `   ${test.method} ${test.endpoint} - Status: ${test.status}, Error: ${test.error || 'Unknown'}`
      );
    });
  }
